...
# Load credentials
# credentials = service_account.Credentials.from_service_account_file(SERVICE_ACCOUNT_FILE) # for local development
...
def insert_into_bigquery(row, table_id):
    ...
    # client = bigquery.Client(credentials=credentials) # for local development
